    ...
    >
      <div className="p-5 flex items-start justify-between">
        <div className="flex items-center gap-4">
           <span className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold font-serif-kr shadow-sm transition-colors ${
             isOpen ? 'bg-indigo-700 text-white' : 'bg-white text-indigo-700 border border-indigo-100'
           }`}>
            {index + 1}
          </span>
      ...
      {/* Expandable Content */}
      <div className={`transition-all duration-300 ease-in-out ${isOpen ? 'max-h-96 opacity-100 p-5 pt-0' : 'max-h-0 opacity-0 overflow-hidden'}`}>
