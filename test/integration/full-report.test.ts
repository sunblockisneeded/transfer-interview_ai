/**
 * 통합 테스트 - 실제 API로 보고서 생성 및 품질 검증
 * 실행: npx tsx test/integration/full-report.test.ts
 */
...
import { validateFullReport, QUALITY_THRESHOLDS } from '../validators/reportValidator.js';
import { FullReport, Professor, CoreConcept } from '../../types.js';
...
async function fetchCurriculum() {
  ...
  const prompt = `
"${UNI} ${DEPT} 교육과정" 또는 "${UNI} ${DEPT} 커리큘럼"을 검색하여 편입 면접 준비 가이드를 작성해주세요.

[필수]
- 실제 과목명 확인 후 작성
- 테이블 형식으로 과목명, 핵심 개념, 면접 예상 질문 정리
...
async function fetchTrends() {
  ...
  const prompt = `
"${UNI} ${DEPT} 편입 면접 후기" 또는 "${UNI} ${DEPT} 편입 합격 수기"를 검색하여 면접 준비 가이드를 작성해주세요.

[출력 형식]
## 실제 면접 질문
## 합격자 공통점
## 준비 체크리스트 (D-30, D-7, D-Day)
`;
...
async function fetchStrategy(currText: string, trendsText: string) {
    ...
    properties: {
      ...
      coreConcepts: {
        ...
        items: {
          type: Type.OBJECT,
          properties: {
            keyword: { type: Type.STRING, description: '개념명 (5단어 이하, 교과서 용어)' },
  ...
  // 핵심: 교수진 분석 제외, 커리큘럼(1-2학년 과목) + 트렌드(기출)만 사용
  const prompt = `
...
- 특정 교수의 연구 분야가 아닌, 학과 공통 핵심 과목 기반
...
2. coreConcepts: 정확히 5개의 핵심 개념
   - 1-2학년 교과서에 나오는 기본 개념
   - 예: 정치학과면 "민주주의", "권력", "현실주의" 등
   - 예: 경영학과면 "STP전략", "재무제표", "SWOT분석" 등

3. questions: 난이도별 각 3개
   - high: 전공 핵심 이론을 실제 사례에 적용 (교과서 심화)
   - medium: 전공 기초 개념 + 최근 시사 이슈 연결
   - low: 지원 동기, 학업 계획, 인성 질문
`;
...
<head>
  ...
  <style>
    ...
    .validation { background: #ecfdf5; border: 1px solid #10b981; padding: 20px; border-radius: 8px; margin-top: 40px; }
    .validation.failed { background: #fef2f2; border-color: #ef4444; }
  </style>
</head>
<body>
...
async function main() {
  ...
  console.log(`🎯 ${UNI} ${DEPT} 보고서 생성 테스트`);
  console.log('═'.repeat(60));
  ...
  try {
    ...
    // 상세 통계
    console.log('\n📋 상세 통계:');
    console.log(`  - 커리큘럼: ${curriculum.text.length}자`);
    console.log(`  - 교수진: ${professors.professors.length}명`);
    console.log(`  - 연구분야 분석: ${professors.majorKnowledgeAnalysis.length}자`);
    console.log(`  - 면접 트렌드: ${trends.text.length}자`);
    console.log(`  - 전략: ${strategy.coreStrategy.length}자`);
    console.log(`  - 핵심 개념: ${strategy.coreConcepts.length}개`);
    console.log(`  - 질문: H${strategy.questions.high.length}/M${strategy.questions.medium.length}/L${strategy.questions.low.length}`);
